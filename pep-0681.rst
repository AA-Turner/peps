...
* It synthesizes an ``__init__`` method using data fields declared
  within the class and its parent classes.
* It synthesizes ``__eq__`` and ``__ne__`` methods.
...
2. Field ordering and inheritance is assumed to follow the rules
   specified in :pep:`557#inheritance`. This includes the effects of overrides
   (redefining a field in a child class that has already been defined
   in a parent class).

```suggestion
3. :pep:`PEP 557 indicates <557#post-init-parameters>` that
...
5. KW_ONLY sentinel values are supported as described in the Python
   docs [#kw-only-docs]_ and bpo-43532 [#kw-only-issue]_.
...
start with a single underscore. This proposal omits that behavior.
...
For example, it automatically adds an ``id`` field (and
